...
const AdminRefundInfo:React.FC<Props> = ({refundInfo}) => {
  ...
  return (
    <div>
      <div><div>{t("AdminRefund:Detail.Item01.Attribute01")}</div><div>{refundInfo.name}</div></div>
      <div><div>{t("AdminRefund:Detail.Item01.Attribute02")}</div><div>{refundInfo.phone}</div></div>
      <div><div>{t("AdminRefund:Detail.Item01.Attribute03")}</div><div>{refundInfo.orderID}</div></div>
