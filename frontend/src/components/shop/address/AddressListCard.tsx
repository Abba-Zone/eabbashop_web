...
const AddressListCard:React.FC<Props> = ({address, cardType, billID, mainID, changeAddress, setAddressList}) => {
  ...
  const clickDeletAddressButton = async () => {
    const newList = await deleteAddress_s(address.addressID);
  ...
  return (
    <div>
      ...
      :
        <div>
          <h3>{name}
            {billID !== address.addressID && mainID !== address.addressID && <button onClick={clickDeletAddressButton}>삭제</button>}
